...
World.events.tick.subscribe(({deltaTime,currentTick}) => {
    try {
        ...
        for (const player of players) {
            ...
            if (itemArray.length) { Commands.run(`execute "${player.nameTag}" ~~~ execute @s[tag=!wardenTrusted,scores={acmtoggle=1}] ~~~ tellraw @a {"rawtext":[{"text":"§¶§c§lUAC ► §6Anti-CBE §d${player.nameTag} §bwas temp-kicked for having §c${itemArray}"}]}`, World.getDimension('overworld')); }
            if (itemArray.length) { Commands.run(`execute "${player.nameTag}" ~~~ execute @s[tag=!wardenTrusted,scores={acmtoggle=1}] ~~~ function tools/asset/cbeitem_gt_warn`, World.getDimension('overworld')); }
        }
    } catch (error) {
        console.warn(error);
    }
