...
Server.command.register(registerInformation, (chatmsg, args) => {
    ...
    if (sender.hasTag('wardenTrusted')) {
        if (worldset.includes(args[0])) {
            ...
            sender.tellraw(`§¶§7[§cWarden§7]§r §¶ §b§lWorld Spawn configured to §e${sender.scoreTest('Worldx')} ${sender.scoreTest('Worldy')} ${sender.scoreTest('Worldz')}§b! Players will be sent here after passing World Border`);
        }
            ...
            sender.tellraw(`§¶§7[§cWarden§7]§r §¶ §b§lCustom World Spawn has been set back to default`);
        }
        else {
            sender.tellraw(`§¶§7[§cWarden§7]§r §¶ §cERROR 2! §6Usage Example §7:§b§l tools.worldspawn [ set | remove ]\n§¶§cINFO ► §bThis tells tools where worldspawn is. Where you're standing will also be where default spawn is now. People will teleport here when crossing world border, or when using the "spawn" chat command.`);
        }
    } else {
        sender.tellraw(`§¶§7[§cWarden§7]§r §¶ §c§lError 4: Only Staff can configure world spawn`);
    }
});