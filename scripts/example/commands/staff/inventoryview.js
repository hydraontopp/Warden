    ...
    try {
        ...
        if (sender.hasTag('wardenTrusted')) {
            if (!args[0]) {
                ...
                //sender.tellraw(`§¶§7[§cWarden§7]§r §¶ §c§lplayerfound : ${playerfound.getName()}`);

                // dump armor
                tellrawStaff(`§d${playername}'s§b inventory checked by §d${sender.name}`);
                sender.runCommand(`execute "${playername}" ~~~ function tools/asset/enchanted_armor_check`);
                ...
                sender.tellraw(`§¶§7[§cWarden§7]§r §¶ §c§lError 7: command failure`);
            }
        } else {
            sender.tellraw(`§¶§7[§cWarden§7]§r §¶ §c§lError 4: Only Staff can use this command`);
        }
        
    } catch (error) {
        sender.tellraw(`§¶§7[§cWarden§7]§r §¶ §c§lError 8: command failure`);
