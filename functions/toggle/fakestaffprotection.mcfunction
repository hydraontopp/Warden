...
execute @s[tag=ownerstatus,scores={SSM=1}] ~~~ tellraw @a[tag=wardenTrusted] {"rawtext":[{"text":"§¶§7[§cWarden§7]§r §¶► §6Fake Staff Protection §bhas been toggled §2ON §bby §d"},{"selector":"@s"}]}
#turn off
...
execute @s[tag=ownerstatus,scores={SSM=2}] ~~~ tellraw @a[tag=wardenTrusted] {"rawtext":[{"text":"§¶§7[§cWarden§7]§r §¶► §6Fake Staff Protection §bhas been toggled §cOFF §bby §d"},{"selector":"@s"}]}
execute @s[tag=ownerstatus,scores={SSM=2}] ~~~ scoreboard players set ssmtoggledummy SSM 0
#Deny Nonstaff
execute @s[tag=!ownerstatus] ~~~ tellraw @s {"rawtext":[{"text":"§¶§7[§cWarden§7]§r §¶► This is ment for owners who have more knowledge with add-ons.\nPreform the Staff Key, then the Ownerstatus Key to unlock this command.\nWhen this is enabled, you must execute the Staff key onto your staff so they may have the wardenTrusted tag.\nFor More info please join our Discord"}]}
execute @s[tag=!ownerstatus] ~~~ function tools/asset/discord
